...
```
This directory would serve as a new Vite + React project called `investments-tracker`.
...
2. The agent definition to use for all future work:
   "You're a frontend developer specialist experienced with stock markets and individual investor portfolio tracking. You keep things simple, ask for confirmation before taking action, show step-by-step reasoning, and explain financial terms when introducing them. If you're unsure, ask."
3. The CSV format the app will consume (semicolon-delimited):
   ...
   - type values: stock, etf, bond, crypto, cash, precious_metal
...
Then create a `public/example-data/portfolio.csv` with 30 rows covering: stocks (USD), ETFs (EUR), crypto (USD), bonds (PLN), precious metals — include multiple buy transactions for the same ticker, multiple sells, and multiple dividend rows.
...
4. In the example CSV, all dividend rows have quantity = 0 and the value is in price. Treat it as: if quantity > 0 then amount = quantity × price, otherwise amount = price. 
5. The aggregator itself only outputs buyAmount and sellAmount. The formula unrealizedGain = currentValue + sellAmount - buyAmount requires a live price, which comes from the price service  
  (will be prompted later). Compute it in the tests by injecting a mock currentValue, and document that the formula lives at the enrichment layer (PortfolioView), not in the aggregator.
...
- Estimated current value - find a way to calculate interest accruals basing on the treasury bonds offer and inflation values
